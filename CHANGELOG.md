...
### Added
- **`helix-cad-roundtrip`** (helix/) — cross-phase CAD orchestrator that sequences the two mirror bridges (`helix-cad-bridge` OUTBOUND: code→STEP+PNG · `helix-cad-ingest` INBOUND: PDF+DXF/DWG→`cad_extract.json`) across HELIX Phase 1-2-3-4, closing the geometry round-trip from RE intake to verified production handoff. A **thin commander** — never parses DXF or writes code-CAD itself; always delegates to a bridge and relays the handoff. Three flows: `--ingest-first` (P1 RE/customer-drawing seed → `helix-p1-requirements`/`reverse-engineering`), `--forward` (P2→P3→P4→fab synthesis, STEP through each block, ICD v3 freeze at `helix-p3-integrate`), `--verify` (post-P4 export-drift loop: read released drawing back + `dwg_dxf_diff.py`, drift treated as directional — DXF-only = real drift, DWG-only superset = expected). Shares the classification label (READ from `helix-p1-validate`, propagated with egress guard, never re-set), the LLM Spatial Blindness gate (bridge Step 2/5 + ingest Step 6 stay CEO Core), and ICD rev-lock with both bridges. Mirror interfaces STEP (out) ↔ cad_extract.json (in) are the only data threaded; no-flag smart router asks lifecycle position and proposes a flow. Static eval 8/8. Skill count 232 → 233.
