...
Chạy toàn bộ kiểm chứng "chứng minh script đúng" trước khi phát hành/commit:
  1. Fixture cài lỗi chủ đích (golden/fixtures/): seed tốt phải PASS, seed xấu phải
     FAIL đúng 7 lỗi, seed vỡ cấu trúc phải bị SCHEMA chặn, cross 2 nguồn phải bắt lệch.
  2. Golden set giải tích (golden/step/): --regen trích lại actual-seed bằng freecadcmd
     (kiểm cả extractor); không --regen thì so actual đã có (chỉ kiểm validator).

Được gọi bởi pre-commit hook (scripts/hooks/pre-commit) khi thay đổi chạm
scripts/extract/ | schemas/ | golden/. Chạy tay:  python scripts/extract/run_battery.py [--regen]
...
def main():
    ...
    if not os.path.isdir(FIX):
        sys.stderr.write("[!] Thiếu golden/fixtures/\n"); sys.exit(3)
