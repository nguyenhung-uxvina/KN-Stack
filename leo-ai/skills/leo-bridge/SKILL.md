...
- **Gate = deterministic theo denylist, KHÔNG LLM suy đoán:** chỉ chặn đúng term có trong `leo_bridge/denylist.yaml`. Tên khí tài / mã dự án / **lớp tàu-tàu ngầm** mới → **phải thêm tay**, nếu không sẽ lọt `THUONG` ra cloud. Quy tắc đặt: cụm tiếng Việt → `defense_terms`; tên riêng/lớp tàu (Kilo, Gepard…) → `codenames` (case-sensitive, né "Kilogram"/"kilo" thường); token lẻ dễ đụng generic ("đạn" ↔ "bạc đạn") → dùng **cụm** ("đầu đạn"), đừng thêm token lẻ.
