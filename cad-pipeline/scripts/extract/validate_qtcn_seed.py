...
# MỘT NGUỒN: cad-pipeline/schemas/materials.json (đọc lúc import; mọi script dùng chung qua
...
def require_validated(seed_path, force=False, out=sys.stderr):
    ...
    if not os.path.exists(rep_path):
        problem = ("chưa qua validator — chạy:\n"
                   "    python cad-pipeline/scripts/extract/validate_qtcn_seed.py --seed \"%s\"" % seed_path)
    else:
        try:
