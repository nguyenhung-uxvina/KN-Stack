...
> Ngày ban hành: ……/2026
> Hiện thực tham chiếu trong KN-Stack: `cad-pipeline/scripts/extract/validate_qtcn_seed.py` (xem Phụ lục A)
...
|---|---|---|
| Inventor BOM export (.xlsx/.csv) | `cad-pipeline/scripts/extract/bom_xlsx_to_seed.py` | field-verified — **nguồn VÀNG** (Mass/QTY/Material Inventor tự tính) |
| Inventor trong app (iLogic) | `cad-pipeline/scripts/inventor/Export_QTCN_Package.iLogic.vb`, `cad-pipeline/scripts/extract/inventor_batch_export.iLogic.vb` | viết theo API chuẩn, chưa chạy thật |
| Inventor headless (Apprentice) | `cad-pipeline/scripts/extract/inventor_apprentice_extract.py` | **skeleton 0.1.0 — chưa qua G3, chưa được dùng sản xuất** |
| STEP/IGES/FCStd | `cad-pipeline/scripts/extract/freecad_extract.py` | field-verified (BM-01, 279 solid) |
| DXF/DWG 2D | `cad-pipeline/scripts/extract/parse_mech_drawing.py` | field-verified (VTI, BM-01) |
| Gộp 2 nguồn | `cad-pipeline/scripts/extract/merge_qtcn_seeds.py` | field-verified |
...
`python cad-pipeline/scripts/extract/validate_qtcn_seed.py --seed <seed.json> [...]` — chạy
**kiểm cấu trúc theo `cad-pipeline/schemas/qtcn-seed.schema.json`** (JSON Schema 2020-12,
...
`cad-pipeline/scripts/extract/self_check.bat`.

**Pipeline MỘT LỆNH**: `python cad-pipeline/scripts/extract/run_pipeline.py --dir <_QTCN_export/<asm>>
...
**Harness tầng 3 (đầu ra AI — §7)**: `python cad-pipeline/scripts/extract/trace_numbers.py --doc
...
**Chiều dài hàn v0.2**: `FC_FILE=<asm.step> freecadcmd cad-pipeline/scripts/extract/weld_length.py`
...
**Vòng phản hồi thực địa (giai đoạn 4)**: schema `cad-pipeline/schemas/qtcn-actuals.schema.json` +
`cad-pipeline/scripts/extract/record_actuals.py` — ghi giờ công/vật tư/NDT THẬT (append-only, kiểm
...
**Dashboard 1 trang**: `python cad-pipeline/scripts/extract/pipeline_dashboard.py --root
...
`cad-pipeline/golden/step/` có 3 case **giải tích** (đáp án tính tay chính xác tuyệt đối):
