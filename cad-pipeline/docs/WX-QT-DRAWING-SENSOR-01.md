...
| D2-04, D3-03, D1-11 | `cad-pipeline/scripts/inventor/Drawing_Check_Live.iLogic.vb` (External Rule, xuất drawing_live_report.json) | rule đã viết — chưa chạy thật trên Inventor |
| D2-04 đường thay thế (không cần Inventor) | `cad-pipeline/scripts/extract/weld_length.py` phân loại cặp interference/trùng-hình-học từ STEP qua FreeCAD | field-tested Tong lap: 1274 cặp cài nhau + 209 cặp trùng chỗ |
| D2-05, D3-07 | đã thêm vào `Drawing_Check_Live.iLogic.vb` (occurrence trôi; đếm weld symbol) | rule đã viết — chưa chạy thật |
| HU-01…HU-04 | `cad-pipeline/scripts/extract/rhino_check.py` (rhino3dm, không cần cài Rhino) + xuất `*.material-map.json` gắn vật liệu vào seed STEP | field-tested trên fixture cài lỗi (cad-pipeline/golden/rhino/, trong battery) |
