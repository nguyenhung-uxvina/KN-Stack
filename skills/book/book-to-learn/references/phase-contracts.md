...
Biến dùng dưới đây (Git Bash — dấu `/`, luôn để trong ngoặc kép vì vault có thư mục chứa dấu cách): `VAULT="D:/Workshop_X"`, `SLUG="<slug>"`, `LEARN="$VAULT/1_Projects/LEARN-$SLUG"`, `BOOK="$VAULT/3_Resources/Books/$SLUG"`, `KN="D:/KN-Stack"`.
...
| L4 | `framework_ung_vien` | orchestrator tự chạy `nlm alias set learn-<slug> <id của btl-<slug>-goc>` + chép sẵn `LEARN/learn/RESOURCES.md` (khuôn `learn-teach:references:RESOURCES-FORMAT`, nguồn ghi đã duyệt ở L2/L3) TRƯỚC khi báo CEO gõ lệnh — S0 của learn-teach không tự kiểm RESOURCES.md/alias đã có hay chưa, nên nếu vẫn hỏi thì CEO trả lời "đã có notebook"; `btl-<slug>-mo-rong` không gắn vào learn-teach, đến qua Claims.md; `LEARN/learn/feynman-<slugify(framework)>.md` | `learn-methodology`, `learn-practice`, `learn-track`; CEO tự gõ `/learn-teach` | `… L4` |
...
VAULT="D:/Workshop_X"
SLUG="<slug>"
LEARN="$VAULT/1_Projects/LEARN-$SLUG"
NOTEBOOK_GOC="<notebook_goc từ _pipeline_state.md>"   # định nghĩa TRƯỚC khi dùng
