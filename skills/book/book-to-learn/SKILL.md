...
- `--resume`: đọc `LEARN/_pipeline_state.md`. Trong các pha CÓ cổng máy (L0, L1, L2, L3, L4, L5, L7 — L6 không có cổng), chạy tiếp pha đầu tiên theo thứ tự chưa có dòng PASS. **Ngoại lệ:** một khi L5 đã PASS, KHÔNG coi "chưa có PASS" của L6 là pha cần chạy (L6 không bao giờ có dòng PASS) — ở lại **L6** cho tới khi đủ điều kiện rời sang L7 (xem mục L6 bên dưới).
- `--from Ln` / `--only Ln`: bắt đầu tại / chỉ chạy pha đó (vẫn chạy cổng). `--only L5 --target <dự án>`: dùng lại một cuốn đã học (BOOK/ tri thức bền đã có sẵn từ chu kỳ trước) để mở một thí nghiệm mới cho một dự án đích khác — bỏ qua L0–L4, vẫn chạy cổng L5.
...
4. Bài học tương tác + ôn giãn cách + đoán-trước-rồi-thử: `learn-teach` có `disable-model-invocation`, nên **CEO tự gõ `/learn-teach`** trong thư mục `LEARN/learn/` (đã có `MISSION.md` viết sẵn từ L0 bước 5). Dặn CEO: pha **S0** của `/learn-teach` KHÔNG tạo notebook mới — `learn-teach` chỉ dùng **một notebook mỗi workspace** (`SOURCING.md` § "Notebook Thường Trực"), nên chạy `nlm alias set learn-<slug> <id của btl-<slug>-goc>` để trỏ thẳng vào notebook toàn văn đã duyệt ở L2. `btl-<slug>-mo-rong` **không** gắn vào `learn-teach`; bằng chứng mở rộng của nó tới CEO qua `Claims.md` (và một truy vấn riêng vào notebook đó khi cần, không qua S0).
