...
4. Bài học tương tác + ôn giãn cách + đoán-trước-rồi-thử: `learn-teach` có `disable-model-invocation`, nên **CEO tự gõ `/learn-teach`** trong thư mục `LEARN/learn/` (đã có `MISSION.md` viết sẵn từ L0 — quy tắc trên). Dặn CEO: khi `/learn-teach` chạy pha **S0**, PHẢI dùng lại notebook đã có — `btl-<slug>-goc`, và `btl-<slug>-mo-rong` nếu đã qua L3 — **không tạo notebook mới**; nguồn đã duyệt ở L2/L3 chính là nguồn S0 cần.
