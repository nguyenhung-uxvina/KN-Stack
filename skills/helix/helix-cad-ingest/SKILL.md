...
> Pipeline: `ingest.py` (read each DXF) → `aggregate.py` (roll up, surface QA flags) → reconcile against PDF BOM via `authoritative_bom.py` (correct codes) → hand `PARTS_MASTER`/`FAB_ROUTING` to `forge-fabrication`.
