...
> **When there is NO DXF (Δ-B single-raster):** for drawings that only exist as a raster/PDF — competitor RE, supplier scan, hand-drawing — use `--single-raster`. It ingests one file with no pair, harvests **TEXT only** (vector PDF text = MED / Tesseract OCR = LOW), and **never auto-extracts geometry** (spatial-blindness law): dimensions/holes/tolerances go to `missing[]` for a human to transcribe + CEO certify. For **MẬT/HẠN-CHẾ** the mode is **human-in-loop, local-only, no cloud vision** (CEO 2026-07-17). This is a scaffold + text-assist, NOT magic geometry reading.
...
1. **`ingest.py`** — Steps 2/4/5 for **DXF or DWG**: `python ingest.py <file.dxf|.dwg|folder> --out <dir> --classification MẬT [--prefer dwg|dxf]` → emits `<filename>.cad_extract.json` + `.md` per part. `.dwg` is read via ODA (see above); a folder with both `<stem>.dwg` + `.dxf` ingests one per stem honoring `--prefer` (default `dwg` = native source). Keys output by **filename** (reliable), records `code_in_dxf` separately at LOW confidence (title-block codes can be stale/copy-pasted), keeps full `raw_text` so nothing is dropped, flags break-view dim overrides into `conflicts[]`. Extend `MATERIALS` for new alloys.
   - **`--single-raster` (Δ-B, NO-DXF fallback):** `python ingest.py <file.pdf|.png|folder> --single-raster --classification MẬT` → same `cad_extract.json` schema from a lone raster. Harvests text via PyMuPDF (`fitz`): selectable PDF text → `pdf-text` **MED**, else Tesseract OCR → `ocr` **LOW**; if `fitz` absent it still emits a scaffold (text empty, all→human). `meta.collection_method="single-raster"`, `meta.human_in_loop_required=true` for MẬT/HẠN-CHẾ. **dims/holes/tolerances = empty**; `missing[]` lists what a human must transcribe → CEO certify (the confidence gate blocks downstream). Never treats a raster as authoritative geometry; never calls cloud vision.
2. **`aggregate.py`** — `python aggregate.py <extract_dir>` → rolls all per-part JSON into `MASTER_BOM.md` + `MASTER_BOM.csv` (ERP import) + `CRITICAL_DIMS.md` + **`drawings_map.md`**, with material/process rollups and a data-quality section listing stale-code / no-code parts.
   - **`drawings_map.md` (Δ-A, bản đồ điều hướng cấp-bộ-hồ-sơ, phương pháp Fairley):** index rẻ "tờ nào chứa gì" đọc <1s TRƯỚC khi lặn vào chi tiết. Mỗi tờ = idx · tên/part · **loại tờ** (Chi tiết/Tổng lắp/Nesting/Mặt bằng/Mặt cắt, suy từ tên+notes) · mã · vật liệu · KT·lỗ · **Conf H/M/L** · conflict · **phân loại** (MẬT/HẠN-CHẾ/THƯỜNG). Mục "🔎 Cần mở bản gốc" liệt kê CHỈ các tờ reliability thấp (dim LOW / có conflict / thiếu mã) — Fairley: chỉ mở `source_drawings/` khi thiếu tin cậy. Bổ trợ MASTER_BOM (part-centric, mua/chế tạo) bằng bản đồ sheet-centric (điều hướng). Tất định, offline.
...
- **`--single-raster` does NOT read geometry** (by design — spatial-blindness): it never populates `dimensions`/`holes` from a raster. Empty dims there is CORRECT, not a bug — the human transcribes them into the extract, then certifies. Treating raster output as measured geometry is the exact failure the mode prevents. For MẬT it refuses cloud vision entirely (human-in-loop).
