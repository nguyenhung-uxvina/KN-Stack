---
name: research
description: End-to-end research pipeline v4.1 with multi-channel source discovery (Exa semantic + Web + YouTube + Authority + Patents), source tier classification (S/A/B/C), cross-validation, and structured NLM extraction. The super-skill combining Exa + WebSearch + /yt-search + /nlm into one workflow with CEO approval gates. Channel 0 uses Exa when available, WebSearch fallback otherwise (--no-exa to force). Use when starting a new research sprint on any technical, defense, or market topic. Triggers on: "research topic", "find sources on", "Exa", "semantic search", "nghien cuu chu de", "tim tai lieu", "nghiên cứu chuyên sâu", "tìm nguồn tham khảo", "patent search", "literature review".
---
